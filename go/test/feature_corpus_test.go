...
//
// NewFreeMusicApi2SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.FreeMusicApi2SDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewFreeMusicApi2SDK(opts)
}
...
func fcCandidates(client *sdk.FreeMusicApi2SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.FreeMusicApi2SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.FreeMusicApi2SDK, name string) any {
