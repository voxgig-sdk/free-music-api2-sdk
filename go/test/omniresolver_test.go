...
import (
	...
	omni "github.com/voxgig-sdk/free-music-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeMusicApi2Error); is {
				out := map[string]any{
					"name":    "FreeMusicApi2Error",
...
func omniCtx(arg any, client *sdk.FreeMusicApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeMusicApi2SDK); is {
			client = live
