...
func TestV1LookupEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREEMUSICAPI__TEST_V__LOOKUP_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func v1_lookupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREEMUSICAPI__TEST_V__LOOKUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREEMUSICAPI__TEST_V__LOOKUP_ENTID"])
