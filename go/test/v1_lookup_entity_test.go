...
func TestV1LookupEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREE_MUSIC_API2_TEST_V1_LOOKUP_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func v1_lookupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"v1_lookup01", "v1_lookup02", "v1_lookup03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("FREE_MUSIC_API2_TEST_V1_LOOKUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREE_MUSIC_API2_TEST_V1_LOOKUP_ENTID"])
