...
func TestV1ListEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FREEMUSICAPI__TEST_V__LIST_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func v1_listBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"v1_list01", "v1_list02", "v1_list03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("FREEMUSICAPI__TEST_V__LIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREEMUSICAPI__TEST_V__LIST_ENTID"])
