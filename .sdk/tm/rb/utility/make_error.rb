...
module FreeMusicApi2Utilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = FreeMusicApi2Context.new({}, nil)
    end
    op = ctx.op || FreeMusicApi2Operation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || FreeMusicApi2Result.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(FreeMusicApi2Error) ? err.msg : err.to_s
    ...
    sdk_err = FreeMusicApi2Error.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(FreeMusicApi2Error)
