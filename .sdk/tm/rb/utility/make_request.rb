...
module FreeMusicApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeMusicApi2Response.new({})
    ...
    if fetch_err
      ...
      response = FreeMusicApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeMusicApi2Response.new(fetched)
    else
