...
// FreeMusicApi2 SDK feature test
//
...
class FtCtrl extends FreeMusicApi2Control
{
...
class FtClient
{
    ...
    private FreeMusicApi2Utility $utility;

    public function __construct(FreeMusicApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): FreeMusicApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeMusicApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new FreeMusicApi2Utility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (FreeMusicApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeMusicApi2Config::make_feature($name);
    ...
    private static function buildUrl(FreeMusicApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeMusicApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeMusicApi2Response($fetched);
        }
    ...
    private function fail(FreeMusicApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeMusicApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
