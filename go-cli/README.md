...
boru-driven command-line client **and** interactive REPL for the FreeMusicApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-music-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./free-music-api2-cli --help
...
./free-music-api2-cli list v1_lookup

# 5. Override the API base URL for a single call
FREE_MUSIC_API2_BASE=https://api.example.com ./free-music-api2-cli list v1_list

# 6. No arguments -> interactive REPL
./free-music-api2-cli
free-music-api2> list v1_list
free-music-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-music-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-music-api2-cli list v1_list
   ```
...
Run with no arguments to open a REPL (prompt `free-music-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./free-music-api2-cli
free-music-api2> list v1_list
free-music-api2> /help
free-music-api2> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-music-api2-cli`. |
...
(`./free-music-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
