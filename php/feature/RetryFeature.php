...
// FreeMusicApi2 SDK retry feature
...
class FreeMusicApi2RetryFeature extends FreeMusicApi2BaseFeature
{
    ...
    public function init(FreeMusicApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreeMusicApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreeMusicApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreeMusicApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
