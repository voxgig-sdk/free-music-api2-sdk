...
class V2SearchEntity
{
    ...
    {
        if ($args) {
            $this->_data = FreeMusicApi2Helpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FreeMusicApi2Helpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws FreeMusicApi2Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FreeMusicApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
