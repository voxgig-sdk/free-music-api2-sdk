...
[MCP](https://modelcontextprotocol.io) server exposing the FreeMusicApi2 SDK as
two agent tools — `free-music-api2_list` and `free-music-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-music-api2-mcp)
make build

# 2. Provide credentials via the environment
export FREE_MUSIC_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-music-api2 \
  -- /absolute/path/to/free-music-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-music-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-music-api2_list: first page of records
{ "entity": "v1_list" }
{ "entity": "v1_list", "query": { } }

// free-music-api2_load: one record by id
{ "entity": "v2_list", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-music-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-music-api2 \
     -- "$PWD"/dist/*/free-music-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-music-api2_list` and `free-music-api2_load` tools now appear
   in new sessions. Ask the agent to *"list v1_list using free-music-api2"*
   and it calls `free-music-api2_list` with `{"entity":"v1_list"}`.
...
```sh
export FREE_MUSIC_API2_APIKEY=sk_live_xxx            # API key
export FREE_MUSIC_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `free-music-api2_list` tool
...
### Call the `free-music-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `free-music-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-music-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_MUSIC_API2_APIKEY` | API key sent with every request. |
| `FREE_MUSIC_API2_BASE` | Optional override of the API base URL. |
...
```sh
./free-music-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-music-api2_load","arguments":{"entity":"v2_list","query":{"id":1}}}}'
```
