// Command FreeMusicApi2-mcp is an MCP server exposing the FreeMusicApi2
// SDK as MCP tools.
//
// Two tools are registered:
//
//	FreeMusicApi2_list  — list records of an entity
//	FreeMusicApi2_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: FREE_MUSIC_API2_APIKEY carries the API key and
	// FREE_MUSIC_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FREE_MUSIC_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FREE_MUSIC_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewFreeMusicApi2SDK(opts)
