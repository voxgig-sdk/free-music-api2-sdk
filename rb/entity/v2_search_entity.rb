# FreeMusicApi2 SDK V2Search entity
...
class V2SearchEntity
  ...
  def data_set(args)
    if args
      @_data = FreeMusicApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FreeMusicApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [V2Search, Hash] the loaded V2Search; raises FreeMusicApi2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreeMusicApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
