...
const __1 = require("..");
const SDK_NAME = 'FreeMusicApi2SDK';
...
function isRunnable(code) {
    return (/new\s+FreeMusicApi2SDK\b/.test(code) ||
        /\bFreeMusicApi2SDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+FreeMusicApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/FreeMusicApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
