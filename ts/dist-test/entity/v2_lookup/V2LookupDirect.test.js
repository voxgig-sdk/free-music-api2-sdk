...
(0, node_test_1.describe)('V2LookupDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FreeMusicApi2SDK({
    ...
    (0, node_test_1.test)('direct-load-v2_lookup', async (t) => {
        ...
        const query = {};
        if (setup.live) {
            params.album_id = 2109615;
        }
        else {
            params.album_id = 'direct01';
        }
        const result = await client.direct({
            path: 'lookup/album/{album_id}',
            method: 'GET',
            params,
            query,
        });
...
function liveScenariosActive() { return false && process.env.FREE_MUSIC_API2_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'FREE_MUSIC_API2_TEST_V2_LOOKUP_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.FreeMusicApi2SDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.FREE_MUSIC_API2_APIKEY,
        }));
        let idmap = env['FREE_MUSIC_API2_TEST_V2_LOOKUP_ENTID'];
    ...
    const client = new __1.FreeMusicApi2SDK({
        base: 'http://localhost:8080',
