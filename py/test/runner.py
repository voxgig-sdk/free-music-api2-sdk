...
class FreeMusicApi2TestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeMusicApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeMusicApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeMusicApi2TestRunner.getenv("FREE_MUSIC_API2_TEST_LIVE")
        override = FreeMusicApi2TestRunner.getenv("FREE_MUSIC_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeMusicApi2TestRunner.getenv(key)
        ...
        explain = FreeMusicApi2TestRunner.getenv("FREE_MUSIC_API2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_MUSIC_API2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeMusicApi2TestRunner._test_control is not None:
            return FreeMusicApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeMusicApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeMusicApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeMusicApi2TestRunner.load_test_control()
...
def load_env_local():
    FreeMusicApi2TestRunner.load_env_local()


def env_override(m):
    return FreeMusicApi2TestRunner.env_override(m)


def entity_data(v):
    return FreeMusicApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeMusicApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeMusicApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeMusicApi2TestRunner.load_test_control()


def live_client_options():
    return FreeMusicApi2TestRunner.live_client_options()


def live_delay_ms():
    return FreeMusicApi2TestRunner.live_delay_ms()
