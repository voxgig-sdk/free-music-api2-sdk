...
class TestV1LookupEntity:
    ...
    def test_should_stream(self):
        ...
        base = FreeMusicApi2SDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set FREE_MUSIC_API2_TEST_V1_LOOKUP_ENTID JSON to run live")
        client = setup["client"]
...
def _v1_lookup_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "FREE_MUSIC_API2_TEST_V1_LOOKUP_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("FREE_MUSIC_API2_TEST_V1_LOOKUP_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("FREE_MUSIC_API2_TEST_LIVE") == "TRUE":
        ...
        client = FreeMusicApi2SDK(helpers.to_map(merged_opts))

    _live = env.get("FREE_MUSIC_API2_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("FREE_MUSIC_API2_TEST_EXPLAIN") == "TRUE",
        "live": _live,
